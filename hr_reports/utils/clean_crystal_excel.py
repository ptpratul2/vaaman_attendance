...
#         hours_12 = 12
#         ampm = "AM"
#         frappe.logger("between 12")
#        elif hours < 12:
#         hours_12 = hours
#         ampm = "AM"
#         frappe.logger("before 12")
#        elif hours == 12:
#         hours_12 = 12
#         ampm = "PM"
#         frappe.logger("after 12")
#        else:
#         hours_12 = hours - 12
#         ampm = "PM"
#         frappe.logger("logged 12")

#    elif is_checkin:
#         hours_12 = 9  # assume 9 AM
#         ampm = "AM"
#    else:
#         hours_12 = 5  # assume 5 PM
#         ampm = "PM"
  
   # Format as YYYY-MM-DD HH:MM:SS AM/PM
#    return f"{date_str} {hours_12:02}:{minutes:02}:00 {ampm}"
   return f"{date_str} 04:02:00 am"
...
def clean_crystal_excel(input_path: str, output_path: str, company: str = None, branch: str = None,
   ...
   while r < total_rows:
       ...
       outtime_row = None
       totals_row = None
       search_end = min(total_rows, r + 12)
       ...
       for col_idx, date_str in date_map.items():
           ...
           # Use field names that match ERPNext Attendance doctype
           rec = {
               "Attendance Date": date_str,
               "Employee Code": emp_code if emp_code else "",
   ...
   # Use ERPNext Attendance field names as columns
   final_cols = ["Attendance Date", "Employee Code", "Employee Name", "Status", "In Time", "Out Time", "Company", "Branch", "Over Time"]
   df_final = pd.DataFrame.from_records(records, columns=final_cols)
  
   # Remove any rows where all critical fields are empty
   df_final = df_final.dropna(subset=["Attendance Date", "Employee Code", "Status"], how="all")
