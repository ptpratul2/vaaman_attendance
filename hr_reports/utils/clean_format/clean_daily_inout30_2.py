# clean_daily_inout30_2.py
"""
Cleaner for Daily IN OUT Report NOV 2025 format (files 4500216931 and 4500216591).

Key behaviour:
- Processes vertical format where each row is an attendance record
- Columns: Contractor, vendorcode, Company, Work Order Name, Employee ID, Ramco_Emp ID,
  Employee Name, Attand Date, Status, In Time, Out time, Total Hour_FILO, Shift Name, Category
- Maps 'Ramco_Emp ID' to Employee (GP NO)
- **Handles duplicate punches**: Groups by employee + date, uses FIRST IN and LAST OUT
- Recalculates Working Hours from first IN to last OUT (handles overnight shifts)
...
def parse_time(date_str: str, time_val, is_checkin: bool) -> Optional[str]:
    ...
    Handles HH:MM:SS, HH:MM, and datetime formats
    """
...
# -------------------------
def clean_daily_inout30_2(input_path: str, output_path: str, company: str = None, branch: str = None) -> pd.DataFrame:
    print("=" * 80)
    print("[clean_daily_inout30_2] Starting - Daily IN OUT Report NOV 2025")
    print(f"[clean_daily_inout30_2] Input: {input_path}")
    print(f"[clean_daily_inout30_2] Output: {output_path}")
    print("=" * 80)
    ...
    print(f"[clean_daily_inout30_2] Raw shape: {df.shape}")
    print(f"[clean_daily_inout30_2] Columns: {df.columns.tolist()}")

    # Validate required columns
    required_cols = ['Attand Date', 'Ramco_Emp ID', 'Employee Name', 'In Time', 'Out time']
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        raise ValueError(f"Missing required columns: {missing_cols}")

    # Step 1: Collect all punch records grouped by employee and date
    print("[clean_daily_inout30_2] Collecting punch records...")
    punch_data = {}  # Key: (ramco_emp_id, date_str), Value: {in_times: [], out_times: [], emp_name: str}

    for idx, row in df.iterrows():
        # Extract values
        date_val = row.get('Attand Date')
        ramco_emp_id = row.get('Ramco_Emp ID')
        ...
        parsed_in_time = parse_time(date_str, in_time, is_checkin=True)
        parsed_out_time = parse_time(date_str, out_time, is_checkin=False)

        if parsed_in_time:
            punch_data[key]['in_times'].append(parsed_in_time)
        if parsed_out_time:
            punch_data[key]['out_times'].append(parsed_out_time)

        if (idx + 1) % 100 == 0:
            print(f"[clean_daily_inout30_2] Processed {idx + 1} raw rows...")

    print(f"[clean_daily_inout30_2] Found {len(punch_data)} unique employee-date combinations")

    # Step 2: Process each employee-date combination (first IN, last OUT)
    print("[clean_daily_inout30_2] Processing attendance records (first IN, last OUT)...")
    records = []
    duplicate_count = 0
    ...
    if duplicate_count > 0:
        print(f"[clean_daily_inout30_2] ⚠️  Handled {duplicate_count} employee-date(s) with multiple punches (using first IN, last OUT)")
    ...
    print(f"[clean_daily_inout30_2] Total records parsed: {len(df_final)}")
    ...
    print(f"[clean_daily_inout30_2] Saved cleaned file: {output_path}")
    print("[clean_daily_inout30_2] Done ✅")
