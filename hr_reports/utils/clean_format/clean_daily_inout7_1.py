# hr_reports/utils/clean_format/clean_daily_inout7_1.py
# =====================================================
...
def convert_xls_to_xlsx(xls_path: str) -> str:
    ...
    print(f"[clean_daily_inout7_1] Converting .xls to .xlsx: {xls_path}")
    ...
    try:
        ...
        wb.save(tmp_path)
        print(f"[clean_daily_inout7_1] Saved temporary .xlsx: {tmp_path}")
        return tmp_path
    except Exception as e:
        print(f"[clean_daily_inout7_1] XLS conversion failed, file might be HTML: {str(e)[:100]}")
...
def parse_html_punch_report(file_path: str) -> pd.DataFrame:
    ...
    print(f"[clean_daily_inout7_1] Parsing HTML table from: {file_path}")
    ...
    for encoding in encodings:
        try:
            with open(file_path, 'r', encoding=encoding) as f:
                content = f.read()
            print(f"[clean_daily_inout7_1] Successfully read file with {encoding} encoding")
            break
    ...
    print(f"[clean_daily_inout7_1] Found {len(parser.rows)} rows in HTML table")
    print(f"[clean_daily_inout7_1] Headers: {parser.headers}")
...
def parse_time_to_datetime(date_str: str, time_str: str) -> Optional[datetime]:
    ...
    try:
        ...
        print(f"[clean_daily_inout7_1] Error parsing datetime: {e}, date: {date_str}, time: {time_str}")
        return None
...
def calculate_working_hours(intime_dt: datetime, outtime_dt: datetime, att_date: str) -> tuple:
    ...
    try:
        ...
        print(f"[clean_daily_inout7_1] Error calculating hours: {e}")
        return None, 0.0
...
# =========================
def clean_daily_inout7_1(input_path: str, output_path: str, company: str = None, branch: str = None, attendance_date: str = None) -> pd.DataFrame:
    """
    ...
    """
    print("=" * 80)
    print("[clean_daily_inout7_1] Starting Punch Report Cleaning")
    print(f"[clean_daily_inout7_1] Input: {input_path}")
    print(f"[clean_daily_inout7_1] Output: {output_path}")
    print(f"[clean_daily_inout7_1] Company: {company}")
    print(f"[clean_daily_inout7_1] Branch: {branch}")
    print("=" * 80)
    ...
    if not attendance_date:
        ...
        if attendance_date:
            print(f"[clean_daily_inout7_1] Extracted date from filename: {attendance_date}")
        else:
            print("[clean_daily_inout7_1] WARNING: Could not extract date from filename, using today")
            attendance_date = datetime.now().strftime("%Y-%m-%d")

    # Step 1: Try to read as Excel file (.xlsx or .xls)
    ...
    if input_path.lower().endswith(".xlsx"):
        try:
            ...
            print(f"[clean_daily_inout7_1] Successfully loaded .xlsx file")
        except Exception as e:
            print(f"[clean_daily_inout7_1] Could not read .xlsx file: {str(e)[:100]}")
            df_raw = None

    # Handle .xls files - try conversion first
        ...
        if xlsx_path:
            ...
            try:
                ...
                print(f"[clean_daily_inout7_1] Successfully loaded converted .xlsx file")
            except Exception as e:
                print(f"[clean_daily_inout7_1] Could not read as Excel, will parse as HTML: {str(e)[:100]}")
                df_raw = None

    # If Excel reading failed or file is HTML, parse as HTML
    if df_raw is None:
        print(f"[clean_daily_inout7_1] Attempting to parse as HTML file...")
        df_raw = parse_html_punch_report(input_path)

    print(f"[clean_daily_inout7_1] Loaded DataFrame shape: {df_raw.shape}")
    print(f"[clean_daily_inout7_1] Columns: {list(df_raw.columns)}")

    # Step 2: Group punches by employee (Safety Pass No)
    print("[clean_daily_inout7_1] Grouping punches by employee...")
    ...
    print(f"[clean_daily_inout7_1] Column mapping: {col_mapping}")
    ...
    print(f"[clean_daily_inout7_1] Found {len(employee_punches)} unique employees")
    ...
    for safety_pass, punches in employee_punches.items():
        ...
        try:
            ...
            if employee_id:
                ...
                print(f"[clean_daily_inout7_1] WARNING: Employee not found for Safety Pass '{clean_safety_pass}' ({emp_name})")
        except Exception as e:
            employees_not_found += 1
            print(f"[clean_daily_inout7_1] ERROR: Exception during employee lookup for Safety Pass {safety_pass} ({emp_name}): {str(e)}")
    ...
    print(f"[clean_daily_inout7_1] Processing summary:")
    ...
    print(f"[clean_daily_inout7_1] Built final DataFrame with {len(df_final)} rows")
    ...
    print(f"[clean_daily_inout7_1] Saved cleaned file: {output_path}")

    # Cleanup temporary file
    if temp_created and os.path.exists(working_file):
        os.unlink(working_file)
        print(f"[clean_daily_inout7_1] Cleaned up temporary file")

    print("[clean_daily_inout7_1] Done ✅")
    return df_final
